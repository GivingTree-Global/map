...
async function generateReport(){
  ...
  try{
    ...
    const fetchPng=(url)=>fetch(url).then(r=>r.ok?r.blob():null).then(b=>b?new Promise(res=>{const fr=new FileReader();fr.onload=()=>res(fr.result);fr.onerror=()=>res('');fr.readAsDataURL(b)}):Promise.resolve('')).catch(()=>'');
